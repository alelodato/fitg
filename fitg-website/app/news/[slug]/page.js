...
export async function generateMetadata({ params }) {
    const articolo = articoli.find((a) => a.slug === params.slug)
    if (!articolo) return {}
    return { title: `${articolo.titolo} — FITG` }
}
...
export default function ArticoloPage({ params }) {
    ...
    const correlati = articoli.filter((a) => a.slug !== params.slug).slice(0, 2)

    return (
        <>
            {/* HERO */}
            <section className="relative overflow-hidden h-[280px] md:h-[420px]">
                <Image src={articolo.img} alt={articolo.titolo} fill className="object-cover brightness-[0.3]" />
                ...
                <div className="absolute inset-0 flex flex-col justify-end px-5 md:px-16 pb-10 md:pb-14">
                    <div className="flex items-center gap-3 mb-4">
                        <span className="font-oswald text-[8px] tracking-[2px] text-gold border border-gold/30 px-2 py-0.5">{articolo.tag}</span>
                        <span className="font-inter text-[11px] text-[#555]">{articolo.data}</span>
                    </div>
            ...
            <section className="bg-[#111] px-5 md:px-16 py-12 md:py-20">
                <div className="grid md:grid-cols-[1fr_320px] gap-12 md:gap-16 max-w-5xl">
                    ...
                    <article>
                        ...
                        {articolo.contenuto.map((blocco, i) => {
                    ...
                    {/* SIDEBAR */}
                    <aside>
                        <div className="bg-[#1A1A1A] border border-gold/20 p-6 mb-6">
                            <div className="font-oswald text-[9px] tracking-[3px] text-gold mb-4">ALTRI ARTICOLI</div>
                            <div className="flex flex-col gap-4">
                                {correlati.map((c) => (
                                    <Link key={c.slug} href={`/news/${c.slug}`} className="group flex gap-3 items-start">
                        ...
                        <div className="bg-[#1A1A1A] border border-gold/20 p-6">
                            <div className="font-oswald text-[9px] tracking-[3px] text-gold mb-3">PROSSIMI EVENTI</div>
                            <p className="font-inter text-[12px] text-[#ccc] leading-relaxed mb-4">
                                Consulta il calendario completo degli eventi FITG.
                            </p>
                            <Link href="/news#eventi" className="font-oswald text-[10px] tracking-[3px] text-gold flex items-center gap-2 hover:gap-3 transition-all">
                                VEDI EVENTI <span className="w-4 h-px bg-gold block" />
                            </Link>
                        </div>
                    </aside>
