...
const categorie = [
    ...
    },
    {
        nome: 'Tactical Race',
        sottotitolo: 'T-Race',
        img: '/tactical-race.jpeg',
        desc: 'La Tactical Race (T-Race) è una divisione che ricrea, in chiave competitiva, scenari operativi ispirati al mondo militare e delle forze di polizia. Durante la gara, l\'atleta si trova ad affrontare un percorso strutturato in cui deve portare a termine mansioni tattiche specifiche, alternando prove di forza fisica e di velocità con fasi di ingaggio su bersagli (sagome) discriminanti, ovvero bersagli che richiedono all\'atleta di distinguere correttamente tra obiettivi validi e non validi, secondo le regole stabilite dal protocollo di gara.',
    },
    {
...
function CategoriaCard({ c, i, isOpen, onToggle }) {
    return (
        <button
            onClick={onToggle}
...
export default function CategorieAccordion() {
    ...
    return (
        <section className="bg-[#111] px-5 md:px-16 py-12 md:py-20">
            ...
            {/* DESKTOP — grid 2x2 (prime 4) + grid 2x2 con ultima cella vuota (restanti 3) */}
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                    {categorie.slice(0, 4).map((c, i) => (
                        <CategoriaCard
                            ...
                            onToggle={() => setOpenDesktop(openDesktop === i ? null : i)}
                        />
                    ))}
                </div>

                {/* GRID — ultime 3 (Tactical Race full width, poi Squadre + Air Soft) */}
                <div className="flex flex-col gap-4">
                    <CategoriaCard
                        c={categorie[4]}
                        i={4}
                        isOpen={openDesktop === 4}
                        onToggle={() => setOpenDesktop(openDesktop === 4 ? null : 4)}
                    />
                    <div className="grid grid-cols-2 gap-4">
                        {categorie.slice(5, 7).map((c, iOffset) => {
                            const i = iOffset + 5
                            return (
                                <CategoriaCard
                                    key={i}
